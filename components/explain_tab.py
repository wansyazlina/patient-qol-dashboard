...
def render_explainability_tab(patient_row, prediction_result):
    ...
    factor_result = (render_top_contributingrisks_section(patient_row,prediction_result))
   
    # -----------------------------------------
    # RAG CLINICAL INTERPRETATION
    # -----------------------------------------

    if factor_result:
        render_clinical_interpretation_section(patient_row,prediction_result,factor_result)
